...
class Predictor():
                ...
                while True:
                    try:
                        ...
                        if mean_acc > max_val_acc:
                            ...
                            self.saver.save(sess, self.config._keyword_save_path)
    ...
    def test_keywords(self):
        ...
        with tf.Session(config=self.gpu_config) as sess:
            ...
            saver.restore(sess, self.config._keyword_save_path)
            sess.run(tf.tables_initializer())
    ...
    def train(self):
        batch = self.iterator.get_next()
        kw_loss, kw_acc = self.predict_keywords(batch)
        ...
        with tf.Session(config=self.gpu_config) as sess:
            ...
            sess.run(tf.global_variables_initializer())
            sess.run(tf.local_variables_initializer())
            kw_saver.restore(sess, self.config._keyword_save_path)
            saver = tf.train.Saver()
            for epoch_id in range(self.config._max_epoch):
                self.iterator.switch_to_train_data(sess)
                cur_step = 0
